...
export default function PrestataireSubmit() {
  ...
  const [form, setForm] = useState({
    ...
    price: '', priceUnit: 'forfait' as string,
    maxPersons: '', includes: '',
    photos: [] as string[],
  });
  ...
  async function handleSubmit() {
    ...
    addService(service);
    // Auto-switch to prestataire mode
      ...
      <div className="min-h-screen flex items-center justify-center px-4">
        <div className="text-center max-w-md">
          ...
          <div className="flex flex-col gap-3">
            ...
            <button onClick={() => { setDone(false); setStep(1); setForm({ type:'', title:'', description:'', location:'', wilaya:'', price:'', priceUnit:'forfait', maxPersons:'', includes:'', photos:[] }); }} className="w-full py-3 border border-gray-300 text-gray-700 rounded-xl font-semibold">
        ...
        {step === 2 && (
          <div className="space-y-4">
            ...
            <div>
              ...
              <input value={form.title} onChange={e => set('title', e.target.value)} className="w-full px-4 py-3 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#0F4C8A]" placeholder="Ex: Transfert aéroport Tunis-Carthage" />
            </div>
            <div>
              <label className="block text-xs font-semibold text-gray-700 mb-1.5">{t('prestataire.submit_desc')} *</label>
              <textarea value={form.description} onChange={e => set('description', e.target.value)} rows={4} className="w-full px-4 py-3 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#0F4C8A] resize-none" placeholder="Décrivez votre service en détail..." />
            </div>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input type="number" min="1" value={form.price} onChange={e => set('price', e.target.value)} className="w-full px-4 py-3 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#0F4C8A]" placeholder="50" />
              </div>
              <div>
            ...
            <div>
              ...
              <textarea value={form.includes} onChange={e => set('includes', e.target.value)} rows={4} className="w-full px-4 py-3 border border-gray-300 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-[#0F4C8A] resize-none" placeholder={"Eau minérale\nClimatisation\nWi-Fi à bord"} />
            </div>
          </div>
        )}
